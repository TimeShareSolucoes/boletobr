    ...
    public class EscritorRemessaCnab400Itau : IEscritorArquivoRemessaCnab400
    {
        ...
        public string EscreverDetalhe(Boleto boleto, int numeroRegistro)
        {
            ...
            string nossoNumeroCarteira =
                boleto.NossoNumeroFormatado.Replace(".", "").Replace("/", "").Replace("-", "").Substring(0, 3);
            string nossoNumeroSequencial =
                boleto.NossoNumeroFormatado.Replace(".", "").Replace("/", "").Replace("-", "").Substring(3, 8);
            string nossoNumeroDigito =
                boleto.NossoNumeroFormatado.Replace(".", "").Replace("/", "").Replace("-", "").Substring(11, 1);
            ...
            try
            {
                ...
                detalhe = detalhe.PreencherValorNaLinha(18, 21,
                    boleto.CedenteBoleto.ContaBancariaCedente.Agencia.PadLeft(4, '0')); // Agência Mantenedora da Conta
                detalhe = detalhe.PreencherValorNaLinha(22, 23, string.Empty.PadRight(2, '0'));
                    // Complemento de Registro
                detalhe = detalhe.PreencherValorNaLinha(24, 28,
                    boleto.CedenteBoleto.ContaBancariaCedente.Conta.PadLeft(5, '0'));
                    // Nro da Conta Corrente da Empresa
                detalhe = detalhe.PreencherValorNaLinha(29, 29, boleto.CedenteBoleto.ContaBancariaCedente.DigitoConta);
                ...
                if (boleto.Moeda == "9" || boleto.Moeda == "09" || boleto.Moeda == "R$" || boleto.Moeda == "REAL")
                    detalhe = detalhe.PreencherValorNaLinha(71, 83, boleto.QuantidadeMoeda.ToString().PadLeft(13, '0'));
                ...
                detalhe = detalhe.PreencherValorNaLinha(84, 86, boleto.CarteiraCobranca.Codigo.PadLeft(3, '0'));
                ...
                detalhe = detalhe.PreencherValorNaLinha(109, 110,
                    boleto.CodigoOcorrenciaRemessa.Codigo.ToString().PadLeft(2, '0')); // Identificação da Ocorrência
                detalhe = detalhe.PreencherValorNaLinha(111, 120,
                    boleto.NumeroDocumento.Replace("-", "").PadLeft(10, '0')); // Nro do Documento de Cobrança
                ...
                detalhe = detalhe.PreencherValorNaLinha(127, 139,
                    boleto.ValorBoleto.ToString("f").Replace(".", "").Replace(",", "").PadLeft(13, '0'));
                ...
                detalhe = detalhe.PreencherValorNaLinha(148, 149,
                    boleto.Especie.Sigla.Equals("DM") ? "01" : boleto.Especie.Codigo.ToString()); // Espécie do Título
                ...
                detalhe = detalhe.PreencherValorNaLinha(161, 173,
                    boleto.JurosMora.ToString().Replace(".", "").Replace(",", "").PadLeft(13, '0'));
                ...
                if (boleto.DataDesconto == DateTime.MinValue)
                    ...
                    detalhe = detalhe.PreencherValorNaLinha(174, 179, boleto.DataDesconto.ToString("ddMMyy"));
                        // Data Limite para Concesão de Desconto

                detalhe = detalhe.PreencherValorNaLinha(180, 192,
                    boleto.ValorDesconto.ToString().Replace(".", "").Replace(",", "").PadLeft(13, '0'));
                    // Valor do Desconto a ser Concedido
                detalhe = detalhe.PreencherValorNaLinha(193, 205,
                    boleto.Iof.ToString().Replace(".", "").Replace(",", "").PadLeft(13, '0'));
                    // Valor do I.O.F. recolhido p/ notas seguro
                detalhe = detalhe.PreencherValorNaLinha(206, 218,
                    boleto.ValorAbatimento.ToString().Replace(".", "").Replace(",", "").PadLeft(13, '0'));
                ...
                detalhe = detalhe.PreencherValorNaLinha(221, 234,
                    boleto.SacadoBoleto.CpfCnpj.Replace(".", "").Replace("/", "").Replace("-", ""));
                    // Nro de Inscrição do Sacado (CPF/CNPJ)
                detalhe = detalhe.PreencherValorNaLinha(235, 264, boleto.SacadoBoleto.Nome.PadRight(30, ' '));
                    // Nome do Sacado
                ...
                detalhe = detalhe.PreencherValorNaLinha(315, 326,
                    boleto.SacadoBoleto.EnderecoSacado.Bairro.PadRight(12, ' ')); // Bairro do Sacado
                ...
                detalhe = detalhe.PreencherValorNaLinha(335, 349,
                    boleto.SacadoBoleto.EnderecoSacado.Cidade.PadRight(15, ' ')); // Cidado do Sacado
                detalhe = detalhe.PreencherValorNaLinha(350, 351,
                    boleto.SacadoBoleto.EnderecoSacado.SiglaUf.PadRight(2, ' ')); // UF do Sacado
                ...
                if (boleto.DataJurosMora == DateTime.MinValue)
                    ...
                    detalhe = detalhe.PreencherValorNaLinha(386, 391, boleto.DataJurosMora.ToString("ddMMyy"));
                        // Data de Mora
